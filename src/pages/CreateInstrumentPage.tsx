...
const CreateInstrumentPage = () => {
  ...
  const handleSaveSurvey = async (surveyData) => {
    ...
    try {
      ...
      if (survey_basis === 'standard') {
        const standardQuestionIds = insertedQuestions
            .map((insertedQ, index) => ({ ...insertedQ, is_standard: localQuestions[index].is_standard }))
            .filter(q => q.is_standard)
            .map(q => q.id);

        if (standardQuestionIds.length > 0) {
            // FIX: Ensure project_id is the new surveyId
            const { error: blueprintError } = await supabase.from('survey_blueprints').insert({ project_id: surveyId, immutable_fields: standardQuestionIds, canonical_blueprint });
 ...
 const handleSelectStandard = (standard) => {
  try {
    // FIX: Correctly access questions from the nested structure
    ...
    const generatedData = {
        ...
        items: questions.map((q: any): LocalQuestion => {
            const baseQuestion: any = {
                ...
                options: q.options || [],
                is_standard: true, // FIX: Ensure the 'lock' is returned
            };
            if (q.type === 'rating') {
