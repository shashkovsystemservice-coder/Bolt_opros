...
async function seedDatabase() {
  console.log('Starting to seed meta_prompts...');

  const { error } = await supabase.from('meta_prompts').insert(seedPrompts);

  if (error) {
    console.error('Error seeding database:', error);
  } else {
    console.log('Successfully seeded 3 meta prompts!');
  }
}
